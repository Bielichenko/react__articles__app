...
export function prepareArticles(articlesFromServer: IArticleCard[]): IArticleCardPrepared[] {
  const preparedArticles = articlesFromServer.map((article: IArticleCard) => {
    let preparedSummary;
